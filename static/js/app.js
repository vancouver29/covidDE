...
// get Cases of each day
renderData = (country_data) => {
  let res = [];
  country_data.forEach((element) => {
    res.push(element.Cases);
  });
  return res;
};

// push worlds data with the corresponding status in different puffers
renderWorldData = (world_data, status) => {
  let res = [];
  world_data.forEach((element) => {
    switch (status) {
      case CASE_STATUS.confirmed:
        res.push(element.TotalConfirmed);
        break;
      case CASE_STATUS.recovered:
        res.push(element.TotalRecovered);
        break;
      case CASE_STATUS.deaths:
        res.push(element.TotalDeaths);
        break;
    }
  });
...
// load worlds data or countries data in different puffers in corresponding days.
...
initDaysChart = async () => {
  ...
  day_chart = new ApexCharts(document.querySelector("#days-chart"), options);
